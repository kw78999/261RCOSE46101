...
---
Method
Offline PCA metadata: project cached keys into a low-dimensional PCA subspace.
Compact block summaries: store sign/range metadata per 64-token KV block.
Online query filtering: project the query once, reject implausible blocks using metadata.
Exact attention on candidates: fetch selected blocks and compute exact QK-softmax-V only there.
```text
